	...
	static class RevertDronePositionPatch {
		...
		static void Init() {
			if(RevertDronePosition.GetValue()) {
				Harmony harmony = new Harmony(MainPlugin.GUID);
				MethodInfo original = typeof(ResourceDrone).GetMethod("SetOnBoard");
				harmony.Unpatch(original, typeof(EnergyDrone).GetMethod("ResourceDrone_SetOnBoard", BindingFlags.Static | BindingFlags.Public));
				harmony.Patch(original, new HarmonyMethod(typeof(RevertDronePositionPatch).GetMethod(nameof(NewPatch), BindingFlags.Static | BindingFlags.NonPublic)));
			}
		}
